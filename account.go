...
//
// An Account is the thing that holds a balance and gets charged. It belongs to
// exactly one owner — a Person, an Org, or a Project — and (Org, Subject) IS the
// money's address: Org names the ledger that holds it, Subject the key within
// that ledger. A deposit credits that address, the gate reads it, a usage debit
// spends it. One address, one answer, every caller.
//
// This package is a pure leaf with no dependencies, because the rule must be
// IDENTICAL in every layer that touches money. The grant (commerce), the gate
// (ai), and the ledger call the same function or they disagree — and when they
// disagree, a customer tops up one account and spends from another.
//
// WHO PAYS IS A PROPERTY OF THE CREDENTIAL. Payer is the only function that
// answers it, and it reads nothing but the credential: no environment variable,
// no allowlist, no precedence rule. The two global lists this replaces
// (PERSONAL_BILLING_ORGS / ORG_BILLING_ORGS) encoded one boolean per org in two
// repos with a precedence between them, and one lied when unset — so the grant
// and the gate could consult "the same" rule and disagree. They did: the grant
// landed on the org account while the gate read the person's, and a funded org
// 402'd its own members.
package account
...
// DefaultOrganization (iam/object/session.go). It is NOT a tenant: it is the
// platform's own org, and it is the home of everyone who has not been placed in a
// real one, because IAM's social/OAuth signup path assigns Owner =
// application.Organization with no tenant logic at all (iam/controllers/auth.go).
// Its members are strangers to each other, so each pays from their OWN account —
// a shared org is not a shared wallet.
//
// This is a constant of the product, not a deployment knob: the same value is
// hardcoded in IAM, and nothing in the fleet configures it.
//
// It is the last fact about billing that does not come from the credential, and it
// survives for ONE reason: tokens minted before IAM shipped the `billing_account`
// claim name no payer, and Payer's fallback still has to answer for them. IAM
// states this same rule authoritatively for every token minted since
// (iam/object/billing_account.go), so this constant is a bridge, not a floor —
// when the last pre-claim token has expired, it deletes with the fallback.
...
// builds the same string from the grant context. IAM shares no code with this
// module, so the grammar is the only thing holding them together: kind is one of
// person|org|project, subject is the Account's own Subject(). Parse is the
// inverse — String ∘ Parse is the identity on every account IAM can mint.
...
// Parse reads a `billing_account` claim back into the Account it names. It is a
// PARSE, not a decision: it never invents an owner, and anything it cannot read —
// an empty claim, an unknown kind, a missing subject, a person or project with no
// name — returns the zero Account, which Payer treats as "the credential named
...
func Parse(claim string) Account {
...
// answer when present — Payer parses it and stops guessing. It reaches a caller two
// ways, both server-minted and neither forgeable: the gateway validates the claim,
// strips any client-supplied copy, and mints X-Billing-Account-Id from it
...
func Payer(c Credential) Account {
	...
	if named := Parse(c.Account); !named.Zero() && named.Org() == owner {
		return named
	}
